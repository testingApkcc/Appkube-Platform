...
const loginServices: (config: any) => LoginServices = (config: any) => {
  ...
  return {
    ...
    oauth: {
      ...
      name:
        oauthEnabled && config.oauth.generic_oauth
          ? config.oauth.generic_oauth.name
          : "OAuth",
      icon: "sign-in",
...
export const LoginServiceButtons = () => {
  const styles = useStyles(getServiceStyles);
  const keyNames = Object.keys(loginServices(config));
  const serviceElementsEnabled = keyNames.filter((key) => {
    const service: LoginService = loginServices(key)[key];
  ...
  const serviceElements = serviceElementsEnabled.map((key) => {
    ...
    return (
      ...
      >
        <i
          className={`btn-service-icon fa fa-${
            service.icon ? service.icon : key
          }`}
        />
