import React, { PureComponent } from 'react';
import { PanelProps } from '@grafana/data';
import './css/style.css';

class AppkubeSuccessAndFailedRequestPanel extends PureComponent<PanelProps> {
  render() {
    return (
      <div className="success-request-graph-panel">
        <div className="success-request-graph-container">
          <span className="title">Success and Failed Request</span>
          <div className="graph-info">
            <div className="graph-status">
              <span className="color failure"></span>
              <span className="name">Failure</span>
            </div>
            <div className="graph-status">
              <span className="color success"></span>
              <span className="name">Success</span>
            </div>
          </div>
          <div className="icon">
            <div className="refresh-icon">
              <img
                src="data:image/png;base64,"
                alt=""
              />
            </div>
            <div className="resize-icon">
              <img
                src="data:image/png;base64,"
                alt=""
              />
            </div>
          </div>
        </div>
      </div>
    );
  }
}

export default AppkubeSuccessAndFailedRequestPanel;
