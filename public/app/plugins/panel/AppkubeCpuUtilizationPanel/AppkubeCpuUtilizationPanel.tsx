import React, { PureComponent } from 'react';
import { PanelProps } from '@grafana/data';
import './css/style.css';
import SingleLineChart from './Components/SingleLineChart';

class AppkubeCpuUtilizationPanel extends PureComponent<PanelProps> {
  render() {
    return (
      <div className="cpu-utilization-panel">
        <div className="cpu-utilization-inner-panel">
          <div className="heading">
            <span>CPU Utilization</span>
            <div className="time-periods">
              <ul>
                <li>Today</li>
                <li>1W</li>
                <li className="active">1M</li>
                <li>3M</li>
                <li>Custom</li>
              </ul>
            </div>
            <div className="buttons">
              <button>
                <img
                  src="data:image/png;base64,"
                  alt=""
                />
              </button>
              <button>
                <img
                  src="data:image/png;base64,"
                  alt=""
                />
              </button>
            </div>
          </div>
          <SingleLineChart />
        </div>
      </div>
    );
  }
}

export default AppkubeCpuUtilizationPanel;
