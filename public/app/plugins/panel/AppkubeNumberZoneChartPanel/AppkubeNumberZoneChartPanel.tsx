import React, { PureComponent } from 'react';
import { PanelProps } from '@grafana/data';
import './css/style.css';
import SingleLineChartWithCurve from './Components/SingleLineChartWithCurve';

class AppkubeNumberZoneChartPanel extends PureComponent<PanelProps> {
  render() {
    return (
      <div className="number-zone-chart-panel">
        <div className="number-zone-chart-inner-panel">
          <div className="heading">
            <span>Port Allocation Error Count</span>
            <div className="buttons">
              <button>
                <img
                  src="data:image/png;base64,"
                  alt=""
                />
              </button>
              <button>
                <img
                  src="data:image/png;base64,"
                  alt=""
                />
              </button>
            </div>
          </div>
          <SingleLineChartWithCurve />
        </div>
      </div>
    );
  }
}

export default AppkubeNumberZoneChartPanel;
